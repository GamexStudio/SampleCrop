...
- (void)viewDidLoad
{
    ...
    self.displayImage.frame = CGRectMake(20, 20, 248, 429);
//    self.originalImage = [UIImage imageNamed:@"dumbo.jpg"];
    self.displayImage.image = self.originalImage;
    NSLog(@"displayImage frame ---- %ld",_displayImage.frame);
    ...
    NSLog(@"cropper frame --- %@",self.cropper);
...
- (IBAction)originalPressed:(id)sender {
    ...
    NSLog(@"cropper image frame  --- %@",self.cropper);
}

- (IBAction)saveToPhotoAlbum:(id)sender {
    UIImageWriteToSavedPhotosAlbum(self.displayImage.image, nil, nil, nil);
}
